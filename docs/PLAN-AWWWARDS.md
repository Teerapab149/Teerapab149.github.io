...
## 9. สรุปตอนปิดงาน (2026-08-01) — ทำอะไรไปบ้าง และเว้นอะไรไว้

**ทำครบแล้ว** L1–L6 · Scene 0 · Scene 0.5 · Scene 1 · Scene 2 · Scene 3 · Scene 4 ·
Scene 5 · Scene 6 · Scene 7 · §5 line-mask (10 จุดบนหน้าแรก) · §6 ทั้งหมด · P7 ทั้งสองสไตล์ชีต

**เว้นไว้โดยตั้งใจ — อย่าหยิบกลับมาโดยไม่คิด**

- **Scene 1 · ไอคอนการ์ดลอยหายใจ + ลูกศรขยับ** — การ์ด `.pick__card` ไม่มีไอคอนและไม่มีลูกศร
  ในมาร์กอัป จะทำต้อง*เพิ่มของประดับใหม่เข้าไปเพื่อให้มันขยับ* ซึ่งขัด §8 บรรทัดสุดท้าย
  ("เปลี่ยนวิธีปรากฏ ไม่ใช่สิ่งที่ปรากฏ") และกิน quota L5 ไปเปล่า ๆ
- **Scene 0 · HUD ป้ายชื่อ section มุมจอ** — มี `%scroll` อยู่แล้วมุมล่าง เพิ่มป้ายที่สองคือ
  ของสองชิ้นแย่งมุมเดียวกัน
- **Scene 0 · eyebrow สไลด์จาก mask** — ใช้ `.rise` แทน · eyebrow เป็น meta จิ๋ว ซึ่ง L3
  อนุญาต fade อยู่แล้ว และการ mask บรรทัดเดียวสั้น ๆ แทบไม่ต่างจาก fade
- **Scene 2 · เงา capsule ขยับตาม** — `box-shadow` เคลื่อนไหวคือการ paint ใหม่ทั้งกล่องทุกเฟรม
  ขัดกติกาข้อ 8 (animate เฉพาะ transform/opacity/clip-path)
- **Scene 6 · body เปลี่ยนพื้นตอน section ครองครึ่งจอ** — ทำเป็น toggle ตอนถึงท้ายเว็บแทน
  พื้น body โผล่เฉพาะตอน overscroll เท่านั้น การผูก IntersectionObserver เพิ่มอีกตัว
  เพื่อสีที่มองไม่เห็น 99% ของเวลาไม่คุ้ม

**ข้อยกเว้นกติกาข้อ 8 ที่ต้องรู้ — `.grow__card` animate `width`/`height`**

กติกาบอกให้ animate เฉพาะ transform/opacity/clip-path แต่กรอบที่ขยายใช้ `width`/`height` จริง
**และต้องเป็นแบบนั้น** — ถ้าใช้ `transform: scale()` แทน (ก) จะกลายเป็น zoom ซึ่งเจ้าของ reject
ไปแล้ว และ (ข) **container query จะหยุดทำงาน** เพราะมันอ่านขนาด layout ไม่ใช่ขนาดหลัง transform
ทั้ง section นี้มีอยู่เพื่อให้ reflow จริงเกิดขึ้น ดังนั้น width คือของที่ยอมแลก

วัดผลกระทบจริงแล้ว ไม่ใช่เดา: **1.19ms ต่อเฟรม** สำหรับ forced reflow (งบเฟรมคือ 16.7ms)
· **CLS ที่ผู้ใช้จริงเจอ = 0** — layout shift ทั้ง 23 รายการถูกมาร์ค `hadRecentInput` จึงไม่เข้าสูตร
(ถ้าวัดด้วย `scrollTo()` จากสคริปต์จะได้ 1.21 เพราะไม่นับเป็น input — **อย่าหลงเชื่อตัวเลขนั้น**)

**ผลเกตรอบปิดงาน**

| เกต | ผล |
|---|---|
| JS ปิดจริง (sandbox ไม่มี allow-scripts) | ผ่าน 5 หน้า — ซ่อนเฉพาะของที่ media/container query ซ่อนตามดีไซน์ |
| overflow แนวนอน 1440/1024/768/390/360 | 0 ทุกหน้า (`scrollWidth === clientWidth`) |
| contrast ทุกคู่ | ผ่าน · คู่คับสุด `--deep-4`/`--paper-3` = 4.73:1 |
| `__i18nCheck()` + สลับ TH/EN | ว่าง · ไป-กลับแล้วโครงสร้างไม่พัง |
| **reduced-motion (stub matchMedia + replay ทุก reduce block)** | ผ่าน — `is-ready` ทันที · `.lm` split 0/10 (ไม่ split ถูกแล้ว) · ไม่มีข้อความไหนถูกซ่อนหรือค้างครึ่งทาง |
| long task ระหว่าง scroll ทั้งหน้า | 2 ครั้ง (77ms, 52ms) จากการเลื่อน 12,510px รวด — ยอมรับได้ |
| CLS | **0** (ดูด้านบน) |

**เกตที่ยังรันไม่ได้ในสภาพแวดล้อมนี้** — CPU throttle 4× และ Lighthouse เต็มรูปแบบ
(ไม่มี DevTools protocol ให้เรียก) · ใช้ตัวแทนที่วัดได้จริงแทน: long task, CLS, จำนวน request (8),
น้ำหนักหน้า (369KB) · **ถ้าจะตรวจให้ครบ ต้องเปิด Lighthouse ใน Chrome เองสักครั้ง**
